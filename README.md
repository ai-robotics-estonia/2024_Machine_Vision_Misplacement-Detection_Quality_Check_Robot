...
# Description
The project develops and tests new machine vision support for collaborative robots (cobots) used in the production of manufacturing industries. The solution enables automated programming of cobots using the learned placement of the workpieces. The solution helps significantly increase the efficiency and safety of the production process and is applicable to an extensive class of production tasks.

## Objectives of the Demonstration Project
The project aims to enhance the efficiency of utilizing cobots in the CNC production process. The project focuses on employing computer vision to develop an automated method for exploiting the cobot's capabilities in assisting with production. The specific objectives include:
...
- **Cobot programming:** The employed cobot in the workshop possesses outdated firmware which was a challenge to communicate with it. The existing solutions for remote control APIs were not well-documented, although we had a complex task. We managed to connect to the cobot and communicate with it successfully after studying many documents and investigating open-source tools as well as predefined codes for the cobot. To enable cobot programming, we developed a comprehensive and scalable Python-based API and a set of libraries that can synthesize programs into low-level URScript instructions executable by the cobot. We connect the computer to the cobot via an Ethernet cable and program the cobot fully automatically.
- **Precise and reliable object detection:** The system is expected to precisely detect, locate, and measure the metal objects to generate a safe program for the cobot. However, there are multiple challenges to fulfilling this task:
 ...
 - **Calibration:** To generate correct objects’ locations for the cobot, the distance between the camera, to the cobot base and its gripper should be calibrated. This process was manual in the beginning, but it was a huge obstacle for our tests since manual calibration was time-consuming and highly inaccurate. Therefore, we developed an automatic eye-to-hand cobot calibration process that transforms the 6D objects’ poses from the camera’s reference frame to the cobot base using linear algebra. Design and development of the automatic calibration was a complicated task due to the complexity of its mathematics and algorithms as well as not having access to the cobot during the development in the lab. We gathered some data at the workshop and used it during the development phase. We stimulated the cobot in Blender and integrated it with our developed solution for testing in the lab.
 - **Test and simulation:** It was not possible to go to the workshop frequently, since it was out of the city and also the cobot was not always available for test and development. To enable testing and development in the laboratory, we developed a framework in RoboDK integrated into the camera, in which the whole system operations could be simulated and tested. This simulation environment loads the detected pieces by the camera and visualizes the cobot operations.
 - **Safety mechanism:** There are several cases that may lead to failure during the cobot operation. Therefore, our developed system should be able to monitor the operations and ensure their safety, precision, and accuracy. We implemented a safety monitoring mechanism through a two-phase system mode: 1) initialization, and 2) execution. In the first mode, the user verifies all the operations, and in the second mode, the system monitors and guarantees the safe progress of the operations.
...
### Data Sources
We captured many images in the workshop environment with the stereo camera in various configurations. Also, we took some metal pieces to the lab for test and development of the object detection methods. Moreover, we received some source codes from the workshop that were being used by the cobot, for development purposes. 

### AI Technologies
The object detection task performed in this project mainly exploits various AI algorithms based on OpenCV. We integrated and customized multiple algorithms for object detection and measurement to achieve high accuracy, including image thresholding, image masking, 6D pose estimation, Hough line transform, contour detection, and homography calculation.
